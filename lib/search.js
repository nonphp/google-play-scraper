...
function checkFinished (opts, savedApps, nextToken, clp) {
  ...
  return request(requestOptions, opts.throttle).then(html =>
    processAndRecur(html, opts, savedApps, clp)
  );
}
...
function initialRequest (opts) {
  ...
  function skipClusterPage (html) {
    ...
    if (match) {
      const innerUrl =
        'https://play.google.com/' + (match[0] || '').split(/"/)[1];
      return request(
        Object.assign(
          {
            url: innerUrl
  ...
  const url = `https://play.google.com/store/search?c=apps&q=${opts.term}&hl=${
    opts.lang
  }&gl=${opts.country}&price=${opts.price}`;
